...
DATASETS: tuple[DatasetCard, ...] = (
    DatasetCard(
        ...
        title="HLS-2 Landsat Operational Land Imager Surface Reflectance Daily Global 30m",
        provider="NASA LP DAAC",
        gee_url="https://developers.google.com/earth-engine/datasets/catalog/NASA_HLS_HLSL30_v002",
        temporal_coverage="2013-present; verify current ingestion and AOI/year counts",
    ...
    DatasetCard(
        ...
        title="HLS Sentinel-2 Multi-spectral Instrument Surface Reflectance Daily Global 30m",
        provider="NASA LP DAAC",
        gee_url="https://developers.google.com/earth-engine/datasets/catalog/NASA_HLS_HLSS30_v002",
        temporal_coverage="2015-present; verify current ingestion and AOI/year counts",
        spatial_resolution="30m",
    ...
    DatasetCard(
        ...
        temporal_coverage="Catalog states 2000-2023; live metadata exposed 2000-2024 on 2026-07-25, so verify expected years",
        spatial_resolution="30 arc-seconds; catalog display reports 1000m",
        bands=("b1",),
        qa_bands=(),
        common_uses=("annual population counts", "population density", "exposure", "change detection"),
        recommended_tasks=("annual_endmember_transition", "zonal_statistics", "change_detection", "export_image"),
        known_limitations=(
            "This is a community asset not managed by Google Earth Engine, so metadata can drift from catalog prose.",
            "b1 is a modeled population count and must not be averaged or bilinearly resampled as if it were continuous density.",
        ),
        scale_notes="Convert b1 counts to density before area-weighted aggregation, then multiply by target pixel area and verify conservation.",
        projection_notes="Use an explicit equal-area target grid and persist annual coverage plus regional population-conservation checks.",
        license_attribution="CC BY 4.0; attribute Oak Ridge National Laboratory / LandScan and the applicable annual dataset citation.",
        last_checked="2026-07-25",
    ),
    DatasetCard(
        dataset_id="external/annual-categorical-landcover",
        title="External annual categorical land-cover asset contract",
        ...
        known_limitations=(
            "The generic card does not assert a particular product's accuracy, license, class map, or availability.",
            "Real private asset IDs and raw redistribution-sensitive rasters must remain outside the public skill repository.",
        ),
        scale_notes="Aggregate reviewed binary class masks to area fractions or use nearest-neighbour semantics; never bilinearly interpolate class codes.",
        projection_notes="Preserve the native categorical grid until class masks are built, then align derived fractions to the fixed target grid.",

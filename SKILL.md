...
26. For large annual workflows, audit collection geometry and filter tiled sources by documented metadata properties before reduction. Use a simple export rectangle plus a grid-aligned study mask. Materialize categorical one-hot area fractions and bounded tile or band-group reductions before assembling model stacks. Declare whether class fractions use whole-cell or valid-area denominators; normalize by `valid_fraction` only behind a reviewed minimum-support gate. If the categorical source is already an Earth Engine asset, try staged internal derivation first; use local preaggregation and re-ingestion only after the bounded internal route fails.
